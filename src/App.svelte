...
import UP2CLONECONF from "./routes/up2clone/configuration/Up2CloneConfiguration.svelte"
import UP2CLONETEMPLATE from "./routes/up2clone/template/Up2CloneTemplate.svelte"
...
import UP2LEARNMONITOR from "./routes/up2learn/monitor/Up2LearnMonitor.svelte" 
import UP2ADMINLOG from "./routes/up2admin/Up2AdminLog.svelte"

import { register,init, getLocaleFromNavigator, _  } from 'svelte-i18n';
import {dataNavigation, cloneNavigation, aiNavigation, learnNavigation, clonegraphmenuoptions, clonesubmenuoptions, datagraphmenuoptions} from '../src/lib/ustore.js'
...
const localizeNavigation = ()=>{
    ...
    for(let i=0; i< $cloneNavigation.length; i++){
        switch($cloneNavigation[i].name){
            ...
            case 'AUTHBUILD':
                ...
                break
            case 'CONFIGURATION':
                $cloneNavigation[i].name = $_('up2clone_nav_conf')
                break
             case 'TEMPLATE':
                $cloneNavigation[i].name = $_('up2clone_nav_template')
                break
        }
    }
    ...
    // Localize LEARN navigation
    for(let i=0; i< $learnNavigation.length; i++){
        switch($learnNavigation[i].name){
            case 'MONITOR':
                $aiNavigation[i].name = $_('up2learn_nav_monitor')
                break;
        }
    }
}
...
<Router {url}>
  <div>
    ...
    <Route path="/UP2CLONE/CONFIGURATION" component={UP2CLONECONF} />
    <Route path="/UP2CLONE/TEMPLATE" component={UP2CLONETEMPLATE} />
    ...
    <Route path="/UP2LEARN/MONITOR" component={UP2LEARNMONITOR} />
